...
Core tests take longer than any other pasta tests, due to the high amount of computational work involved in validating core components.
...
To run only pasta's core tests (after building):
...
To run only pasta's crypto tests (after building):
...
To run only pasta's hash tests (after building):
...
To run only pasta's performance tests (after building):
...
To run only pasta's unit tests (after building):
